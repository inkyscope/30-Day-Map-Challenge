...
ggplot(centroids) +
  ...
  coord_fixed(xlim = c(-2, 2), ylim = c(-2, 2),
              clip = "off") +
  labs(caption = "Source: Longitude of coutnry centroids | Credit: @gkaramanis") +
  theme_void() +
  theme(plot.background = element_rect(fill = "gray98",
                                       color = NA),
        plot.caption = element_text(family = "Montserrat Alternates",
                                    hjust = 0.5),
        plot.margin = margin(20, 20, 20, 20))

ggsave("Indonesia.png", width = 26, height = 27, units = "cm")

### This plot is my favorite one among all the contributions to #30DayMapChallenge. The only change I made is fonts.
```
